...
"""
  rand_tt([T::Type], dims, rks; normalise=false, orthogonal=false, right=true, stable=true, seed=nothing)
  rand_tt([T::Type], dims, rmax::Int; normalise=false, orthogonal=false, right=true, stable=true, seed=nothing)
  rand_tt(x_tt::TTvector{T,N}; ε=1e-3, seed=nothing)

Generate random tensor train (TT) vectors with specified properties.

# Arguments

## Core Parameters
- `T::Type`: Element type (defaults to `Float64`)
- `dims`: Tuple or vector of tensor dimensions along each mode
- `rks`: Vector of TT ranks `[r₀, r₁, ..., r_d]` where `r₀ = r_d = 1`
- `rmax::Int`: Maximum rank (automatically determines `rks` based on dimensions)
- `x_tt::TTvector`: Existing TT vector (for perturbation variant)

## Randomization Options
- `seed::Union{Nothing,Int}`: Random seed for reproducible generation. Uses `Random.default_rng()` when set.

## Normalization and Orthogonalization
- `normalise::Bool=false`: Apply entry-wise normalization scaling
  - If `true`: scales entries by `1/√(dims[i] × rks[i+1])` (right) or `1/√(dims[i] × rks[i])` (left)
- `orthogonal::Bool=false`: Apply QR orthogonalization to TT cores
  - Works independently of `normalise` parameter
  - Ensures numerical stability and canonical forms
- `right::Bool=true`: Direction for normalization and orthogonalization
  - `true`: Right-canonical form (left-orthogonal cores)
  - `false`: Left-canonical form (right-orthogonal cores)

## Stability Control
- `stable::Bool=true`: Apply automatic rank constraint enforcement
  - `true` (default): Use `r_and_d_to_rks()` to ensure ranks respect dimension limits
  - `false`: Use provided ranks as-is, allowing arbitrary border ranks and custom rank sequences

## Perturbation Variant
- `ε::Number=1e-3`: Perturbation strength for `rand_tt(x_tt; ε, seed)` variant

# Returns
- `TTvector{T,N}`: Random TT vector with specified properties

# Variants

1. **Type and dimensions specified**: `rand_tt(T, dims, rks; ...)`
2. **Default Float64 type**: `rand_tt(dims, rks; ...)`
3. **Maximum rank specified**: `rand_tt(dims, rmax; ...)` - determines ranks automatically
4. **Perturbation of existing TT**: `rand_tt(x_tt; ε, seed)` - adds random perturbation

# Implementation Details

- Uses `r_and_d_to_rks()` when `stable=true` to ensure rank constraints respect dimension limits
- When `stable=false`, accepts arbitrary ranks including non-unit border ranks
- QR orthogonalization uses standard `qr()` decomposition with compatibility checks
- Throws `ArgumentError` with helpful messages when orthogonalization is mathematically impossible
- Orthogonality information is tracked but not set in `ttv_ot` (remains zeros)
- Random number generation respects `seed` parameter for reproducibility

# Examples

```julia
# Basic random TT vector
dims = (4, 4, 4)
rks = [1, 3, 3, 1]
v = rand_tt(dims, rks)

# Orthogonal TT vector with seeding
v_orth = rand_tt(dims, rks; orthogonal=true, seed=1234)

# Right-canonical normalized form
v_norm = rand_tt(ComplexF64, dims, rks; normalise=true, orthogonal=true, right=true)

# Using maximum rank
v_maxr = rand_tt(dims, 5; orthogonal=true)

# Arbitrary ranks with custom border ranks (research use)
v_custom = rand_tt(dims, [3, 5, 4, 2]; stable=false)

# Perturbed version of existing TT
v_pert = rand_tt(v; ε=1e-2, seed=5678)
```

# See Also
- `zeros_tt()`: Create zero TT vectors
- `ones_tt()`: Create TT vectors of all ones  
- `orthogonalize()`: Orthogonalize existing TT vectors
- `ttv_decomp()`: Decompose full tensors into TT format
"""
function rand_tt(::Type{T},dims,rks;normalise=false,orthogonal=false,right=true,stable=true,seed=nothing) where T
  ...
  N = length(dims)
  # Apply dimension constraints only when stable=true
  if stable
    rks = r_and_d_to_rks(rks, dims; rmax=maximum(rks))
  else
    # Validate that rks has correct length
    @assert length(rks) == N + 1 "rks must have length $(N+1), got $(length(rks))"
  end
  
  y = zeros_tt(T,dims,rks)
  if orthogonal && right
    # Right-orthogonal: orthogonalize all cores except the last (orthogonality center)
    for i=1:N-1
    # Check orthogonality compatibility if requested
    # Need rks[i] <= dims[i] * rks[i+1] for QR to work
      if rks[i] > dims[i] * rks[i+1]
...
function rand_tt(dims,rmax::Int;normalise=false,orthogonal=false,right=true,stable=true, seed=nothing)
  ...
  # For rmax variant, always apply r_and_d_to_rks since user specified max rank, not exact ranks
  rks = r_and_d_to_rks(rks,dims;rmax=rmax)
  return rand_tt(dims,rks;normalise=normalise,orthogonal=orthogonal,right=right,stable=true,seed=seed)
end

function rand_tt(x_tt::TTvector{T,N};ε=convert(T,1e-3),seed=nothing) where {T,N}
  # Set random seed if provided
  rng = Random.default_rng()
  if isnothing(seed)
    Random.seed!(rng)
