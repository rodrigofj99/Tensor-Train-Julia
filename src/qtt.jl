...
function qtt_polynom(coef,d;a=0.0,b=1.0)
  ...
  out.ttv_vec[1][1,2,:] = [φ(t₁,k) for k in 0:p-1]
  for k in 2:d-1
    for j in 0:p-1
      out.ttv_vec[k][j+1,1,j+1] = 1.0
      for i in 0:p-1
        tₖ = h*2^(d-k)
        out.ttv_vec[k][i+1,2,j+1] = binomial(i,i-j)*tₖ^(i-j)
      end
    end
  end
...
function qtt_cos(d;a=0.0,b=1.0,λ=1.0)
  ...
  t₁ = a+h*2^(d-1) #convention : coarsest first
  out.ttv_vec[1][1,2,:] = [cos(λ*π*t₁); -sin(λ*π*t₁)]
  for k in 2:d-1
...
function qtt_sin(d;a=0.0,b=1.0,λ=1.0)
  ...
  t₁ = a+h*2^(d-1) #convention : coarsest first
  out.ttv_vec[1][1,2,:] = [sin(λ*π*t₁); cos(λ*π*t₁)]
  for k in 2:d-1
    out.ttv_vec[k][:,1,:] = [1 0;0 1]
    tₖ = h*2^(d-k)
    out.ttv_vec[k][:,2,:] = [cos(λ*π*tₖ) -sin(λ*π*tₖ); sin(λ*π*tₖ) cos(λ*π*tₖ)]
...
function toeplitz_to_qtto(α,β,γ,d)
  # Operator cores have layout (L, i_out, i_in, R).
  out = zeros_tto(2,d,3)
  ...
  for i in 1:2
    for j in 1:2
      out.tto_vec[1][1,i,j,:] = [id[i,j];J[j,i];J[i,j]]
      for k in 2:d-1
