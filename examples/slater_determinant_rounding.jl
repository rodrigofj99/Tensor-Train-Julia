...
"""
Slater Determinant Rounding Experiments

Compares different rounding methods on Slater determinant wavefunctions with increasing
...
"""
Slater Determinant Rounding Experiment
"""
function slater_determinant_experiment(;
    N = 20,                                    # Number of orbitals
    n_elec = 10,                               # Number of electrons
    ...
    mkpath("out/slater_determinant")
    filename = "out/slater_determinant/N$(N)_nelec$(n_elec).json"

    if !force_rerun && isfile(filename)
        println("=== Slater Determinant Experiment ===")
        ...
        if !params_match
            ...
            println("Loaded:  N=$(results["N"]), n_elec=$(results["n_elec"])")
            println("Current: N=$N, n_elec=$n_elec")
            ...
            for key in keys(results)
                if key ∉ ["N", "n_elec", "target_ranks", "block_rks_list", "n_realizations", "seed"]
            ...
            create_slater_plot(results, ranks_loaded, block_rks_loaded)

            return results
        end
    end

    println("=== Slater Determinant Rounding Experiment ===")
    println("Number of orbitals: N = $N")
    println("Number of electrons: n_elec = $n_elec")
    ...
    println("--- Creating Slater determinant ---")
    Random.seed!(seed)

    # Create random Slater determinant as a full tensor, then convert to TT
    C = random_slater(n_elec, N)
    @show size(C)
    psi = ttv_decomp(C; tol=1e-6)
    psi = psi / norm(psi)
    psi_norm = norm(psi)

    println("Created Slater determinant with $(length(psi.ttv_vec)) cores")
    ...
    # Create plots
    println("\n--- Creating plots ---")
    create_slater_plot(results, target_ranks, block_rks_list)
...
"""
function create_slater_plot(results, rank_list, block_rks_list)
    mkpath("out/slater_determinant/plots")
    ...
    p = plot(xlabel="Target Rank",
             ...
             legend=:topright,
             size=(800, 600),
             title="Slater Determinant Rounding (N=$(results["N"]), n_elec=$(results["n_elec"]))",
             dpi=150)
    ...
    display(p)
    sleep(5)
    savefig(p, "out/slater_determinant/plots/N$(results["N"])_nelec$(results["n_elec"]).png")
    println("Plot saved")
end
...
Run the experiment
"""
function run_slater_experiments(; force_rerun = false)
    println("="^70)
    println("SLATER DETERMINANT ROUNDING EXPERIMENTS")
    println("="^70)

    results = slater_determinant_experiment(
        N = 18,
        n_elec = 9,
        target_ranks = [32,48,64,96,128],
...
println("Running Slater determinant rounding experiments...")
global slater_results = run_slater_experiments()
