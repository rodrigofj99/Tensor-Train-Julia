...
for rX in eachindex(ranks_X)
    ...
    for λ = 1:λ_max
        ...
        for T = 1:num_realizations
            ...
            gtt2[λ][:,:,T],_ = GTT(rng, X, dims, k(λ), normalization="spherical", batch=λ, T=gtt_type)
    ...
    for R in eachindex(ranks)
        ...
        for T = 1:num_realizations
            ftt[R][:,:,T],_ = TTR(rng, X, dims, [ranks[R]], k_max, normalization="none", T=ftt_type)
            ftt[R][:,:,T] = sqrt(k_max)*ftt[R][:,:,T]*C # correct for 1/sqrt(k) in TTR

            ttpr[R][:,:,T],_ = TTPR(rng, X, dims, [ranks[R]], k_max ÷ ranks[R], ranks[R], normalization="none", T=ttpr_type)
            ttpr[R][:,:,T] = sqrt(k_max ÷ ranks[R])*ttpr[R][:,:,T]*C # correct for 1/sqrt(k) in TTPR

            
            ftt2[R][:,:,T],_ = TTR(rng, X, dims, [ranks[R]], k_max, normalization="spherical", T=ftt_type)
            ftt2[R][:,:,T] = sqrt(k_max)*ftt2[R][:,:,T]*C # correct for 1/sqrt(k) in TTR

            ttpr2[R][:,:,T],_ = TTPR(rng, X, dims, [ranks[R]], k_max ÷ ranks[R], ranks[R], normalization="spherical", T=ttpr_type)
    ...
    p1 = plot(range(1,λ_max), α_gtt, label="GTT - RG", yscale=:log10, linestyle=:dot, seriescolor=:black, legend=:right)
    plot!(p1, range(1,λ_max), α_gtt2, label="GTT - CSD", yscale=:log10, linestyle=:dot, seriescolor=:red)

    for R in eachindex(ranks)
        plot!(p1, range(1,λ_max), median(α_ftt[R,:,:],dims=2), label="TT($(ranks[R])) - RG", yscale=:log10, linestyle=:dash)   
        plot!(p1, range(1,λ_max), median(α_ftt2[R,:,:],dims=2), label="TT($(ranks[R])) - CSD", yscale=:log10, linestyle=:dash)
    end

    for R in eachindex(ranks)
        plot!(p1, range(1,λ_max), median(α_ttpr[R,:,:],dims=2), label="TT(k(λ)/$(ranks[R]), $(ranks[R])) - RG", yscale=:log10, linestyle=:solid)
        plot!(p1, range(1,λ_max), median(α_ttpr2[R,:,:],dims=2), label="TT(k(λ)/$(ranks[R]), $(ranks[R])) - CSD", yscale=:log10, linestyle=:solid)
    end
    ...
    p2 = plot(range(1,λ_max), β_gtt, label="GTT - RG", yscale=:log10, linestyle=:dot, seriescolor=:black, legend=:bottomright)
    plot!(p2, range(1,λ_max), β_gtt2, label="GTT - CSD", yscale=:log10, linestyle=:dot, seriescolor=:red)

    for R in eachindex(ranks)
        plot!(p2, range(1,λ_max), median(β_ftt[R,:,:],dims=2), label="TT($(ranks[R])) - RG", yscale=:log10, linestyle=:dash)   
        plot!(p2, range(1,λ_max), median(β_ftt2[R,:,:],dims=2), label="TT($(ranks[R])) - CSD", yscale=:log10, linestyle=:dash)
        
    end

    for R in eachindex(ranks)
        plot!(p2, range(1,λ_max), median(β_ttpr[R,:,:],dims=2), label="TT($(k_max/ranks[R]), $(ranks[R])) - RG", yscale=:log10, linestyle=:solid)
        plot!(p2, range(1,λ_max), median(β_ttpr2[R,:,:],dims=2), label="TT($(k_max/ranks[R]), $(ranks[R])) - CSD", yscale=:log10, linestyle=:solid)
    end

    xlabel!(p2, "Subspace dimension λ")
    #plot!(p2, legendfontsize=6)
