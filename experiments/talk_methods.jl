...
for rX in eachindex(ranks_X)
    ...
    for R in eachindex(ranks)
        plot!(p1, range(1,λ_max), median(α_ttpr[R,:,:],dims=2), label="TT(k(λ)/$(ranks[R]), $(ranks[R]))", yscale=:log10, linestyle=:solid)
    end
    ...
    for R in eachindex(ranks)
        plot!(p2, range(1,λ_max), median(β_ttpr[R,:,:],dims=2), label="TT($(k_max/ranks[R]), $(ranks[R]))", yscale=:log10, linestyle=:solid)
    end
