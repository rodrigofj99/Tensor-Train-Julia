using TensorTrains
using LinearAlgebra
using Plots
using Statistics
using LaTeXStrings
include("utilities.jl")
include("sketches.jl")

# Number of independent runs
num_realizations = 10

#Random.seed!(2)
rng = MersenneTwister(2)

# embedding dimensions
k = r -> 3*r+8 # embedding dimension as a linear function of rank

λ_max = 100 #Subspace dimension
k_max = k(λ_max) #Embedding dimension

# Ranks and physical dimensions of tensors
ranks = [1, 14] #[1, 2, 17] #divisors of k_max
ranks_X = [10]
d = 2 # dimensions
N = 50 # cores

# Tensor train and embedding dimension
dims = ntuple(i -> d, N)

X = SizedArray{Tuple{λ_max}, TTvector{Float64,N}}(undef)

# Store times
gtt_times = zeros(λ_max)
gtt2_times = zeros(λ_max)
ftt_times = zeros(length(ranks), λ_max)
ttpr_times = zeros(length(ranks), λ_max)

# Time the sketching functions
for rX in eachindex(ranks_X)
    for λ = 1:λ_max
        tmp1 = vcat(1, fill(ranks_X[rX], N-1), 1)
        tmp2 = [reverse(cumprod(reverse(dims)))..., 1]
        R = [min(tmp1[i], tmp2[i]) for i in eachindex(tmp1)]
        X_λ = tt_randn(rng, dims, R, normalization="none", orthogonal=true)
        if rX == 2
            X_λ = (1 + rand()*10^(-2))*X_λ
        end
        X[λ] = X_λ/norm(X_λ)
    end

    A = zeros(λ_max, λ_max)
    for λ = 1:λ_max
        A[λ,λ] = tt_dot(X[λ],X[λ])
        for j = λ+1:λ_max
            A[λ,j] = tt_dot(X[λ],X[j])
            A[j,λ] = conj(A[λ,j])
        end
    end

    # This factor orthogonalizes the space: X*C is orthogonal. We sketch X but
    # look to compute Omega'*X*C
    C = inv(cholesky(A).U)

    # Collect all the sketches for further post-processing
    gtt_type = Float64 #ComplexF64
    ftt_type = ComplexF64
    ttpr_type = ComplexF64

    gtt = Vector{Array{gtt_type,3}}(undef, λ_max)
    ftt = Vector{Array{ftt_type,3}}(undef, length(ranks))
    ttpr = Vector{Array{ftt_type,3}}(undef, length(ranks))

    gtt2 = Vector{Array{gtt_type,3}}(undef, λ_max)
    ftt2 = Vector{Array{ftt_type,3}}(undef, length(ranks))
    ttpr2 = Vector{Array{ftt_type,3}}(undef, length(ranks))

    # Timing the GTT function
    for λ = 1:λ_max
        gtt[λ] = zeros(gtt_type, k(λ), λ, num_realizations)
        gtt2[λ] = zeros(gtt_type, k(λ), λ, num_realizations)

        for T = 1:num_realizations
            gtt_times[λ] = @elapsed gtt[λ][:,:,T], _ = GTT(rng, X, dims, k(λ), normalization="none", batch=λ, T=gtt_type)
            gtt[λ][:,:,T] = gtt[λ][:,:,T]*C[1:λ, 1:λ]

            gtt2_times[λ] = @elapsed gtt2[λ][:,:,T], _ = GTT(rng, X, dims, k(λ), normalization="none", orthogonal=true, batch=λ, T=gtt_type)
            gtt2[λ][:,:,T] = gtt2[λ][:,:,T]*C[1:λ, 1:λ]
        end
    end

    # Timing the TTR and TTPR functions
    for R in eachindex(ranks)
        ftt[R] = zeros(ftt_type, k_max, λ_max, num_realizations)
        ttpr[R] = zeros(ttpr_type, k_max, λ_max, num_realizations)

        ftt2[R] = zeros(ftt_type, k_max, λ_max, num_realizations)
        ttpr2[R] = zeros(ttpr_type, k_max, λ_max, num_realizations)

        for T = 1:num_realizations
            ftt_times[R, λ] = @elapsed ftt[R][:,:,T], _ = TTR(rng, X, dims, [ranks[R]], k_max, normalization="spherical", T=ftt_type)
            ftt[R][:,:,T] = sqrt(k_max)*ftt[R][:,:,T]*C # correct for 1/sqrt(k) in TTR

            ttpr_times[R, λ] = @elapsed ttpr[R][:,:,T], _ = TTPR(rng, X, dims, [ranks[R]], k_max ÷ ranks[R], ranks[R], normalization="spherical", T=ttpr_type)
            ttpr[R][:,:,T] = sqrt(k_max ÷ ranks[R])*ttpr[R][:,:,T]*C # correct for 1/sqrt(k) in TTPR

            ftt2_times[R, λ] = @elapsed ftt2[R][:,:,T], _ = TTR(rng, X, dims, [ranks[R]], k_max, normalization="spherical", orthogonal=true, T=ftt_type)
            ftt2[R][:,:,T] = sqrt(k_max)*ftt2[R][:,:,T]*C # correct for 1/sqrt(k) in TTR

            ttpr2_times[R, λ] = @elapsed ttpr2[R][:,:,T], _ = TTPR(rng, X, dims, [ranks[R]], k_max ÷ ranks[R], ranks[R], normalization="spherical", orthogonal=true, T=ttpr_type)
            ttpr2[R][:,:,T] = sqrt(k_max ÷ ranks[R])*ttpr2[R][:,:,T]*C # correct for 1/sqrt(k) in TTPR
        end
    end

    #
    # --- Begin Plotting Times ---
    #

    # Plot the times for each function
    p1 = plot(range(1, λ_max), gtt_times, label="GTT", yscale=:log10, linestyle=:dot, seriescolor=:black, legend=:topright)
    plot!(p1, range(1, λ_max), gtt2_times, label="OGTT", yscale=:log10, linestyle=:dot, seriescolor=:red)

    for R in eachindex(ranks)
        plot!(p1, range(1, λ_max), ftt_times[R, :], label="TT($(ranks[R]))", yscale=:log10, linestyle=:dash)
        plot!(p1, range(1, λ_max), ttpr_times[R, :], label="TTPR($(ranks[R]))", yscale=:log10, linestyle=:solid)
        
        plot!(p1, range(1, λ_max), ftt2_times[R, :], label="OTT($(ranks[R]))", yscale=:log10, linestyle=:dash)
        plot!(p1, range(1, λ_max), ttpr2_times[R, :], label="TTPR OTT($(ranks[R]))", yscale=:log10, linestyle=:solid)
    end
    
    xlabel!(p1, "Subspace dimension λ")
    ylabel!(p1, "Time (seconds)")
    display(p1)

    #
    # --- End Plotting Times ---
    #
end
